# -*- coding: utf-8 -*-
"""
将 STARmap h5ad 数据转换为 STdGCN 标准 TSV/CSV 输入格式。

输出目录会包含 `sc_data.tsv`、`sc_label.tsv`、`ST_data.tsv` 和
`coordinates.csv` 四类文件，分别对应单细胞表达、单细胞标签、空间表达
和空间坐标。脚本假设输入文件已经下载到 `data/starmap/`。
"""
import scanpy as sc
import pandas as pd
import os

# 1. 加载 h5ad 数据。单细胞数据和空间数据的 obs/var 结构可能不同，
#    因此后续分别转换表达矩阵、标签和坐标。
...
# 2. 创建 STdGCN 训练脚本期望的输出目录。
output_dir = 'data/starmap_tsv'
os.makedirs(output_dir, exist_ok=True)

# 3. 保存单细胞表达矩阵：行是细胞，列是基因。
#    稀疏矩阵需要先转为稠密数组，否则 pandas 无法直接按 TSV 写出。
...
# 4. 保存单细胞标签。不同数据集的细胞类型列名不统一，
#    因此按常见列名自动查找，找不到时回退到第一列并提示用户检查。
...
if cell_type_col is None:
    ...
    cell_type_col = sc_adata.obs.columns[0]  # 默认使用第一列，避免脚本直接中断。
...
# 6. 保存空间坐标。优先使用 AnnData 标准的 `obsm['spatial']`，
#    若不存在，则从 obs 中尽量推断 x/y 坐标列。
